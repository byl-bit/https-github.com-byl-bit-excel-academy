...
export const applySearchFilter = <T extends Record<string, unknown>>(
  ...
  if (options.grade) {
    filtered = filtered.filter(
      (item) => String(item.grade || "") === String(options.grade),
    );
  }
  ...
  if (options.section) {
    filtered = filtered.filter(
      (item) => String(item.section || "") === String(options.section),
    );
  }
  ...
  if (options.status) {
    filtered = filtered.filter(
      (item) => String(item.status || "") === String(options.status),
    );
  }
  ...
  if (options.role) {
    filtered = filtered.filter(
      (item) => String(item.role || "") === String(options.role),
    );
  }
