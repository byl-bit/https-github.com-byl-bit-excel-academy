...
export const AuthProvider = ({ children }: { children: ReactNode }) => {
  ...
  useEffect(() => {
    try {
      ...
      if (legacyUsers && legacyUsers.length > 2 * 1024 * 1024) {
        console.info(
          "Clearing large legacy local users data to free up quota.",
        );
  ...
  const login = async (
    ...
    try {
      ...
      try {
        ...
        console.warn(
          "LocalStorage quota exceeded. Storing user session without photo.",
          err,
        );
        try {
          ...
          localStorage.setItem(
            STORAGE_KEYS.CURRENT_USER,
            JSON.stringify(strippedUser),
          );
        } catch (innerErr) {
          console.error(
            "Failed to save even stripped user session to localStorage:",
            innerErr,
          );
    ...
    try {
      ...
      if (grade && section) {
        const checkRes = await fetch(
          `/api/users?role=student&grade=${grade}&section=${section}`,
        );
        const classUsers = checkRes.ok ? await checkRes.json() : [];
        const cleanNormalize = (s: string) =>
          (s || "").trim().replace(/\s+/g, " ").toLowerCase();
        const targetFullName = [firstName, middleName, lastName]
          .filter(Boolean)
          .join(" ")
          .trim()
          .replace(/\s+/g, " ")
      ...
      const fullName = [clean(firstName), clean(middleName), clean(lastName)]
        .filter(Boolean)
        .join(" ");
      ...
      const newUser = {
        ...
        email: isUpdate
          ? placeholder.email
          : `${finalStudentId.toLowerCase()}@excel.edu`,
      ...
      if (postRes.ok) {
        return {
          success: true,
          message:
            "Registration submitted. Await admin approval to access the system.",
          studentId: finalStudentId,
        };
      } else {
        return {
          success: false,
          message:
            data.error || data.message || "Registration failed server-side.",
        };
      }
    } catch (e) {
  ...
  const registerTeacher = async (
    ...
    try {
      ...
      if (postRes.ok) {
        return {
          success: true,
          message: "Teacher registration submitted. Await admin approval.",
          teacherId,
        };
      }
  ...
  const getPendingUsers = () => {
    ...
    console.warn(
      "getPendingUsers in AuthContext is deprecated. Use direct API fetch.",
    );
    return [];
  };
