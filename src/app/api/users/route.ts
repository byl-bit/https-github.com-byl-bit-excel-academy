  ...
  try {
    ...
    const mappedUsers = (users || []).map((u: unknown) => {
      ...
      return {
        ...
        gender:
          normalizeGender(getString(u, "gender") || getString(u, "sex")) ||
          null,
  ...
  try {
    ...
    const mapUserToDb = (u: Record<string, unknown>) => {
      const dbUser: Record<string, unknown> = {
        ...
        roll_number:
          typeof getString(u, "rollNumber") !== "undefined"
            ? Number(getString(u, "rollNumber"))
            : typeof getString(u, "roll_number") !== "undefined"
              ? Number(getString(u, "roll_number"))
              : undefined,
        updated_at: getString(u, "updatedAt") || new Date().toISOString(),
      };

      // Remove undefined keys to avoid inserting explicit undefined values
      Object.keys(dbUser).forEach(
        (k) => dbUser[k] === undefined && delete dbUser[k],
      );
    ...
    if (Array.isArray(body)) {
      ...
      for (const item of body) {
        ...
        const itemStudentId = isRecord(item)
          ? getString(item, "studentId") || getString(item, "student_id")
          : undefined;
        const itemTeacherId = isRecord(item)
          ? getString(item, "teacherId") || getString(item, "teacher_id")
          : undefined;
        ...
        if (!existing) {
          ...
          const rollVal = isRecord(item)
            ? getString(item, "rollNumber") || getString(item, "roll_number")
            : undefined;
          const gradeVal = isRecord(item)
            ? getString(item, "grade")
            : undefined;
          const nameVal = isRecord(item)
            ? getString(item, "name") || getString(item, "fullName")
            : undefined;
          ...
          if (roleVal === "student" && rollVal && gradeVal) {
            const { data: identityMatch } = await supabase
              ...
              .eq(
                "section",
                isRecord(item) ? getString(item, "section") : undefined,
              )
              .eq("roll_number", Number(rollVal))
              .limit(1);

            if (identityMatch && identityMatch.length > 0)
        ...
        if (typeof itemRoll !== "undefined" && itemRoll !== "") {
          ...
          if (isNaN(rNum) || rNum < 1 || rNum > 100) {
            skippedUsers.push({
              ...item,
              error: "Roll number out of range (1-100)",
            });
            continue;
          }
        }
    ...
    const { data: existingCheck } = await supabase
      ...
      .select("id")
      .or(
        `email.eq.${body.email},student_id.eq.${body.studentId},teacher_id.eq.${body.teacherId}`,
      )
      .limit(1);
...
export async function PUT(request: Request) {
  try {
    ...
    const isApproving =
      updates["status"] === "active" && targetUser["status"] === "pending";
    const isStudent =
      targetUser["role"] === "student" || updates["role"] === "student";

    if (isApproving && isStudent) {
      // Logic: Link to pre-imported placeholder if exists
      const fullName =
        (updates["fullName"] as string) ||
        (updates["name"] as string) ||
        (targetUser["name"] as string);
      const grade =
        (updates["grade"] as string) || (targetUser["grade"] as string);
      const section =
        (updates["section"] as string) || (targetUser["section"] as string);
      const rollNumber =
        (updates["roll_number"] as string) ||
      ...
      if (placeholders && placeholders.length > 0) {
        ...
        const mergedUser = {
          ...
          student_id:
            ...
            (placeholder as Record<string, unknown>)["student_id"],
          status: "active",
    ...
    const willUpdateRoll =
      typeof updates["rollNumber"] !== "undefined" ||
      typeof updates["roll_number"] !== "undefined";
    const willUpdateName =
      typeof updates["fullName"] !== "undefined" ||
      typeof updates["name"] !== "undefined";
    const willUpdateGradeSection =
      typeof updates["grade"] !== "undefined" ||
      typeof updates["section"] !== "undefined";

    // Check duplicates for students
    if (
      (willUpdateRoll || willUpdateName || willUpdateGradeSection) &&
      (targetUser["role"] === "student" || updates["role"] === "student")
    ) {
      const checkRoll = String(
        updates["rollNumber"] ??
          updates["roll_number"] ??
          targetUser["roll_number"] ??
          targetUser["rollNumber"] ??
          "",
      );
      const checkGrade = String(updates["grade"] ?? targetUser["grade"] ?? "");
      const checkSection = String(
        updates["section"] ?? targetUser["section"] ?? "",
      );
    ...
    if (typeof updates["grade"] !== "undefined")
      updatedData.grade = updates["grade"];
    if (typeof updates["section"] !== "undefined")
      updatedData.section = updates["section"];
    if (updates["gender"])
      updatedData.gender = normalizeGender(updates["gender"] as string) || null;
    ...
    try {
      ...
      if (typeof updates["email"] !== "undefined") {
        notifications.push({
          ...
          target_id: id,
          target_name:
            updatedUser?.name || (targetUser["name"] as string) || "",
        });
      }
