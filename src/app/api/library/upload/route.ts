...
export async function POST(request: Request) {
  try {
    ...
    console.log(
      `[Library Upload] Processing file: ${file.name}, Size: ${file.size}, Type: ${file.type}`,
    );
    ...
    if (!client) {
      ...
      return NextResponse.json(
        { error: "Server configuration error: Database client unavailable." },
        { status: 500 },
      );
    }
    ...
    if (
      ...
      console.warn(
        `[Library Upload] Bucket ${bucketName} not found. Attempting to create...`,
      );
      ...
      if (supabaseAdmin) {
        ...
        if (createError) {
          console.error(
            "[Library Upload] Failed to create bucket:",
            createError,
          );
          return NextResponse.json(
            {
        ...
        console.error(
          "[Library Upload] Bucket missing and no Admin key to create it.",
        );
    ...
    return NextResponse.json(
      { error: e.message || "Server error during upload" },
      { status: 500 },
    );
  }
}
