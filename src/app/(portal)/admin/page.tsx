...
export default function AdminPage() {
  ...
  const [processingResetId, setProcessingResetId] = useState<string | null>(
    null,
  );
  ...
  const handleApproveUser = async (userId: string) => {
    try {
      ...
      const payload: any = String(userId).startsWith("ST-")
  ...
  const handleRejectUser = async (userId: string) => {
    ...
    try {
      ...
      const query = String(userId).startsWith("ST-")
        ? `?studentId=${encodeURIComponent(userId)}`
  ...
  const handleUpdateUser = async (userId: string, data: any) => {
    try {
      ...
      const payload = String(userId).startsWith("ST-")
        ? { studentId: userId, ...data }
        : { id: userId, ...data };
      const res = await fetch("/api/users", {
        method: "PUT",
        headers,
      ...
      if (res.ok) {
        logActivity({
          ...
          action: "Updated User",
          category: "user",
          details: `Updated profile for ${userId}`,
        });
      ...
      notifyError("Update failed: Connection error");
    }
  };
    ...
    try {
      ...
      if (res.ok) {
        ...
        logActivity({
          ...
          category: "user",
          details: `Enrolled ${app.familyFullName} from Admissions`,
        });
        refresh(true);
    ...
    try {
      ...
      if (res.ok) {
        logActivity({
          ...
          category: "user",
          details: `Rejected application ID ${id}`,
        });
        refresh(true);
        info("Application rejected");
      }
    } catch (e) {
      notifyError("Failed to reject application");
    }
  };
  ...
  const handlePublishResult = async (result: any) => {
    try {
      ...
      const payload = result.studentId
    ...
    try {
      ...
      if (res.ok) {
        logActivity({
          userId: user?.id || "",
          userName: user?.name || "Admin",
          action: "Approved Result",
          category: "result",
          details: `Approved results for ${name}`,
        });
  ...
  const handleApproveSubjectMarks = async (
    studentKey: string,
    subjectName: string,
  ) => {
    try {
      ...
      notifyError("Subject approval failed");
    }
  };

  const handleApproveManyResults = async (
    keys: string[],
    teacherName: string,
  ) => {
    try {
      ...
      if (res.ok) {
        logActivity({
          userId: user?.id || "",
          userName: user?.name || "Admin",
          action: "Approved Bulk Results",
          category: "result",
          details: `Approved ${keys.length} results submitted by ${teacherName}`,
        });
        refresh(true);
      }
    } catch (e) {
      notifyError("Bulk approval failed");
    }
  };
    ...
    try {
      ...
      if (res.ok) {
        logActivity({
          userId: user?.id || "",
          userName: user?.name || "Admin",
          action: "Rejected Result",
          category: "result",
          details: `Rejected pending results for ${name}`,
        });
        refresh(true);
      }
    } catch (e) {
  ...
  const handleUnlockResult = async (id: string) => {
    if (
      !confirm(
        "Unlock this result for teacher editing? It will be removed from published and moved back to drafts.",
      )
    )
      return;
    try {
      ...
      if (res.ok) {
        logActivity({
          ...
          category: "result",
          details: `Unlocked result ID ${id} for editing`,
        });
        success("Result unlocked and moved to drafts");
        refresh(true);
      }
    } catch (e) {
      notifyError("Unlock failed");
    }
  };
    ...
    try {
      ...
      if (res.ok) {
        logActivity({
          ...
          category: "library",
          details: `Added resource: ${book.title}`,
        });
        refresh(true);
      }
    } catch (e) {
      notifyError("Failed to add resource");
    }
  };
  ...
  const handleDeleteBook = async (id: string) => {
    ...
    try {
      ...
      notifyError("Failed to delete resource");
    }
  };
  ...
  const handleToggleMaintenance = () => {
    ...
    logActivity({
      ...
      category: "system",
      details: `Maintenance mode set to ${next}`,
    });
  };
  ...
  const handleAddSubject = async (name: string) => {
    ...
    logActivity({
      ...
      category: "system",
      details: `Added ${name} to subjects`,
    });
    refresh();
  };
  ...
  const handleDeleteSubject = async (name: string) => {
    ...
    logActivity({
      ...
      category: "system",
      details: `Removed ${name} from subjects`,
    });
    refresh();
  };
  ...
  const handleChangeAdminPassword = async (pass: string) => {
    try {
      ...
      if (res.ok) {
        ...
        notifyError(
          "Unauthorized: You must be an admin to change this password (re-login may be required).",
        );
        return false;
      } else {
  ...
  const handleResetSystem = async () => {
    if (
      !confirm(
        "CRITICAL: You are about to RESET the entire system. This will delete ALL students, results, and data.\n\nA backup PDF of all results will be generated first. Continue?",
      )
    )
      return;
    ...
    try {
      ...
      if (resultsArray.length > 0) {
        ...
        resultsArray.forEach((result) => {
          ...
          doc.setFontSize(10);
          doc.text("STUDENT PROGRESS REPORT", pageWidth / 2, 18, {
            align: "center",
          });
          ...
          (result.subjects || []).forEach((sub: any, sIdx: number) => {
            ...
            doc.text((sub.marks || 0) >= 40 ? "PASS" : "FAIL", 160, y, {
              align: "center",
            });
            y += 8;
          });
          ...
          doc.setFontSize(8);
          doc.text(
            "System Backup Generated Record",
            pageWidth / 2,
            pageHeight - 10,
            { align: "center" },
          );
        ...
        doc.save(
          `Excel_Academy_FULL_System_Backup_${new Date().toISOString().split("T")[0]}.pdf`,
        );
      } else {
        doc.save(
          `Academy_Backup_${new Date().toISOString().split("T")[0]}.pdf`,
        );
      }
      ...
      if (
        !confirm(
          "Backup PDF downloaded. Do you want to proceed with DELETING all data now?",
        )
      )
        return;
      ...
      logActivity({
        userId: actorId,
        userName: user?.name || "Admin",
        action: "SYSTEM RESET COMPLETE",
        category: "system",
        details: `Full factory reset performed - All academic modules cleared`,
      });
      logActivity({
        userId: user?.id || "",
        userName: user?.name || "Admin",
        action: "SYSTEM RESET",
        category: "system",
        details: `RESET PERFORMED - Backup generated`,
      });
      window.location.reload();
    } catch (e) {
      notifyError("Reset failed during backup or execution.");
    }
  };
  ...
  const handleRejectAllStudents = async () => {
    const pendingStudents = users.filter(
      (u) => u.status === "pending" && u.role === "student",
    );
    if (pendingStudents.length === 0) return;
    if (
      !confirm(
        `Are you sure you want to REJECT ALL ${pendingStudents.length} pending student applications? This cannot be undone.`,
      )
    )
      return;
    ...
    try {
        ...
        await Promise.all(
          batch.map(async (u) => {
            try {
              ...
              const query = String(u.id).startsWith("ST-")
                ? `?studentId=${encodeURIComponent(u.id)}`
      ...
      logActivity({
        userId: user?.id || "",
        userName: user?.name || "Admin",
        action: "Bulk Reject Students",
        category: "user",
        details: `Rejected ${successCount} students`,
      });
      success(`Rejected ${successCount} applications`);
      refresh(true);
    } catch (e) {
      notifyError("Bulk rejection failed");
    }
  };

  const handleRejectAllTeachers = async () => {
    const pendingTeachers = users.filter(
      (u) => u.status === "pending" && u.role === "teacher",
    );
    if (pendingTeachers.length === 0) return;
    if (
      !confirm(
        `Are you sure you want to REJECT ALL ${pendingTeachers.length} pending teacher applications? This cannot be undone.`,
      )
    )
      return;
    ...
    try {
      ...
      logActivity({
        userId: user?.id || "",
        userName: user?.name || "Admin",
        action: "Bulk Reject Teachers",
        category: "user",
        details: `Rejected ${successCount} teachers`,
      });
  ...
  const handleRejectAllAdmissions = async () => {
    ...
    if (
      !confirm(
        `Are you sure you want to REJECT ALL ${admissions.length} admission applications? This cannot be undone.`,
      )
    )
      return;
    ...
    try {
      const processBatch = async (batch: any[]) => {
        await Promise.all(
          batch.map(async (app) => {
            try {
              const res = await fetch(`/api/admissions?id=${app.id}`, {
                method: "DELETE",
              });
              if (res.ok) successCount++;
            } catch (e) {
      ...
      logActivity({
        ...
        category: "user",
        details: `Rejected ${successCount} admission applications`,
      });
  ...
  const handleRejectAllResults = async () => {
    ...
    if (
      !confirm(
        `Are you sure you want to REJECT ALL ${pendingResults.length} pending result submissions? This cannot be undone.`,
      )
    )
      return;
    ...
    try {
      ...
      if (res.ok) {
        logActivity({
          userId: user?.id || "",
          userName: user?.name || "Admin",
          action: "Bulk Reject Results",
          category: "result",
          details: `Rejected ${keys.length} result submissions`,
        });
