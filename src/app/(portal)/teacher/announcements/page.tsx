...
export default function TeacherAnnouncementsPage() {
  ...
  const [votes, setVotes] = useState<
    Record<
      string,
      { likes: number; dislikes: number; myVote?: "like" | "dislike" }
    >
  >({});
  ...
  const handleVote = (id: string | number, vote: "like" | "dislike") => {
    ...
    const updated = {
      ...votes,
      [key]: { likes: newLikes, dislikes: newDislikes, myVote: newVote },
    };
    setVotes(updated);
        ...
        <div className="flex items-center gap-4 p-2">
          ...
          <div>
            ...
            <p className="text-sm text-slate-500">
              Official updates, circulars and notices for teachers.
            </p>
          </div>
        </div>
      </Card>
      ...
      <div className="space-y-6">
        {announcements.length > 0 ? (
          announcements.map((announcement) => (
            <Card
              key={announcement.id}
              className="overflow-hidden border-t-4 border-t-indigo-500 hover:shadow-lg transition-all bg-white border-slate-100"
            >
                ...
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    ...
                    <div className="flex items-center gap-2 mt-2">
                      <span
                        className={`text-[10px] font-black px-3 py-1 rounded-full uppercase tracking-widest bg-indigo-50 text-indigo-700 border border-indigo-100`}
                      >
                        {announcement.type}
                      </span>
              ...
              <div className="p-6 pt-4">
                ...
                {announcement.type === "event" && (
                  <div className="flex items-center gap-4 mt-4 pt-3 border-t border-slate-100">
                    ...
                    >
                      <ThumbsUp
                        className={`h-4 w-4 ${votes[String(announcement.id)]?.myVote === "like" ? "fill-indigo-600" : ""}`}
                      />
                    ...
                    >
                      <ThumbsDown
                        className={`h-4 w-4 ${votes[String(announcement.id)]?.myVote === "dislike" ? "fill-red-600" : ""}`}
                      />
                      <span>
          ...
          <Card>
            <div className="flex flex-col items-center justify-center py-16 text-center">
              ...
              <p className="text-slate-400 text-sm italic">
                There are no new faculty announcements at the moment.
              </p>
            </div>
          </Card>
        )}
