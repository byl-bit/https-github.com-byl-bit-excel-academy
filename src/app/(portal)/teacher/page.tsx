...
export default function TeacherPage() {
    ...
    const fetchData = async () => {
      ...
      try {
        ...
        const myAllocations = allAllocations.filter((a: any) => {
          ...
          const tName = String(
            a.teacherName || a.teacher_name || "",
          ).toLowerCase();

          return (
            (tId && (tId === uId || tId === uTId)) ||
            (!tId &&
              tName &&
              uName &&
        ...
        const contextGrade = isHomeroomTeacher
          ? user.grade
          : myAllocations[0]?.grade;
        const contextSection = isHomeroomTeacher
          ? user.section
          : myAllocations[0]?.section;
        ...
        if (contextGrade && contextSection) {
          ...
          const [usersRes, resRes] = await Promise.all([
            fetch(
              `/api/users?role=student&grade=${contextGrade}&section=${contextSection}&status=active`,
            ),
            fetch(
              `/api/results?grade=${contextGrade}&section=${contextSection}`,
              {
                headers: { "x-actor-role": "teacher", "x-actor-id": user.id },
              },
            ),
  ...
  const hasResponsibility =
    allocations.length > 0 || (!!user.grade && !!user.section);
    ...
    return (
      <Card className="text-center py-16 flex flex-col items-center justify-center border-dashed border-2 border-slate-200 bg-slate-50/50">
        ...
        <p className="text-slate-500 max-w-sm mt-2">
          Contact the administrator to be assigned as a Homeroom Teacher or
          given Subject Allocations.
        </p>
      </Card>
    );
  }
