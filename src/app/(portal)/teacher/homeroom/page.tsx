...
export default function HomeroomPage() {
  ...
  const allSubjects = Array.from(
    new Set(results.flatMap((r) => r.subjects?.map((s: any) => s.name) || [])),
  ).sort();
  ...
  const generateBulkReports = async () => {
    ...
    try {
        ...
        .map((r) => {
          ...
          const studentProfile = students.find((s) => {
            ...
            if (
              rDisplayId &&
              sDisplayId &&
              String(rDisplayId) === String(sDisplayId)
            )
              return true;
            return false;
          });
          ...
          return {
            ...r,
            studentName:
              studentProfile.fullName || studentProfile.name || rName, // Sync Name
            studentId:
              studentProfile.studentId ||
              studentProfile.student_id ||
              rDisplayId ||
              "N/A", // Sync ID (Prefer display ID)
            gender: normalizeGender(
              studentProfile.gender || studentProfile.sex || r.gender,
            ), // Sync Gender
            rollNumber:
              studentProfile.rollNumber || r.rollNumber || r.roll_number || "-",
      ...
      for (let i = 0; i < reportData.length; i++) {
        ...
        doc.setFont("helvetica", "normal");
        doc.text(
          `Name: ${result.studentName || result.student_name || "Student"}`,
          20,
          70,
        );
        doc.text(
          `ID: ${result.studentId || result.student_id || "N/A"}`,
          20,
          80,
        );
        ...
        doc.setFont("helvetica", "bold");
        doc.text(
          `Total Score: ${Number.isInteger(result.total) ? result.total : result.total.toFixed(1)}`,
          20,
          y,
        );
        ...
        y += 15;
        const isPass =
          result.promotedOrDetained === "PROMOTED" || result.result === "PASS";
        doc.setTextColor(
          isPass ? 22 : 220,
          isPass ? 163 : 38,
          isPass ? 74 : 38,
        ); // Green-600 or Red-600
        doc.text(
          `FINAL RESULT: ${result.promotedOrDetained || result.result}`,
          20,
          y,
        );
        ...
        doc.text(
          "Generated via Excel Academy Admin Portal",
          pageWidth / 2,
          bottomY,
          { align: "center" },
        );
    ...
    return (
      <div className="flex flex-col items-center justify-center py-20 text-center">
        ...
        <h2 className="text-xl font-bold text-blue-900">
          Not Assigned as Homeroom Teacher
        </h2>
        <p className="text-blue-500 max-w-md mt-2">
          You must be assigned to a specific Grade and Section as a Homeroom
          Teacher to access this portal.
        </p>
      </div>
    );
  }
