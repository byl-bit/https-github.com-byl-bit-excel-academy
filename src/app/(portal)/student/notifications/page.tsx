      ...
      <div className="flex flex-col md:flex-row items-center justify-between gap-6">
        <div>
          ...
          <p className="text-slate-500 font-bold mt-1">
            Stay updated with the latest school activity.
          </p>
        </div>
      ...
      {notifications.length === 0 ? (
        ...
        <div className="grid gap-4">
          {notifications.map((notif) => (
            <Card
              key={notif.id}
              className={`relative overflow-hidden transition-all duration-300 hover:shadow-md border-none ${notif.is_read ? "bg-white/60 opacity-80" : "bg-white shadow-sm ring-1 ring-blue-100"}`}
            >
              {!notif.is_read && (
              ...
              <CardContent className="p-6">
                <div className="flex items-start gap-4">
                  <div
                    className={`p-3 rounded-2xl ${notif.is_read ? "bg-slate-100" : "bg-blue-50"}`}
                  >
                  ...
                  <div className="flex-1 space-y-1">
                    <div className="flex items-center justify-between">
                      <h4
                        className={`font-black text-lg ${notif.is_read ? "text-slate-600" : "text-slate-800"}`}
                      >
                        {notif.action}
                      </h4>
                    ...
                    <div className="flex items-center gap-3 pt-3">
                      ...
                      {notif.category === "result" && (
                        <Button
                          ...
                          size="sm"
                          asChild
                          className="h-8 rounded-lg text-[10px] font-black uppercase tracking-wider border-emerald-100 text-emerald-600 hover:bg-emerald-50"
                        >
                      ...
                      {notif.category === "announcement" && (
                        <Button
                          ...
                          size="sm"
                          asChild
                          className="h-8 rounded-lg text-[10px] font-black uppercase tracking-wider border-amber-100 text-amber-600 hover:bg-amber-50"
                        >
