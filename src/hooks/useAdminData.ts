...
export function useAdminData() {
    ...
    async (
      ...
      try {
        ...
        if (category === "all" || category === "essentials") {
          ...
          const fetchedSettings = settingsRes.ok
            ? await settingsRes.json()
            : {};
          const fetchedAnnouncements =
            (announceRes.ok ? await announceRes.json() : []) || [];
          ...
          setSubjects(
            Array.isArray(fetchedSubjects) && fetchedSubjects.length
              ? fetchedSubjects
              : [
                  "Mathematics",
                  "English",
        ...
        if (
          ...
          if (category === "all" || category === "results") {
            tasks.push(fetch("/api/results", authHeaders));
          ...
          for (let i = 0; i < results.length; i++) {
            ...
            try {
              ...
              if (type === "users") {
                ...
                setAllResults(
                  Object.keys(published).map((k) => {
                    ...
                    return {
                      ...
                      gender:
                        normalizeGender(r["gender"] ?? r["sex"] ?? null) ||
                        null,
                      ...
                      published_at:
                        r["published_at"] ?? r["publishedAt"] ?? null,
                    } as PublishedResult;
                  }),
                );
                ...
                setPendingResults(
                  Object.keys(pending).map((k) => {
                    ...
                    return {
                      ...
                      submitted_by:
                        r["submitted_by"] ?? r["submittedBy"] ?? null,
                      submitted_at:
                        r["submitted_at"] ?? r["submittedAt"] ?? null,
                    } as PendingResult;
                  }),
                );
                ...
                setResetRequests(
                  (data as any[]).map((r) => {
                    const userData = Array.isArray(r.users)
                      ? r.users[0]
                      : r.users;
                    return {
                      id: r.id,
          ...
          setLoadedCategories((prev) => {
            ...
            if (category === "all") {
              ["users", "results", "admissions", "resources"].forEach((c) =>
                next.add(c),
              );
            } else {
              next.add(category);
  ...
  useEffect(() => {
    ...
    if (
      users.length > 0 &&
      (allResults.length > 0 || pendingResults.length > 0)
    ) {
      const resultsRecord: Record<string, any> = {};
      allResults.forEach((r) => {
        if (r.studentId) resultsRecord[r.studentId] = r;
      });
      const pendingRecord: Record<string, any> = {};
      pendingResults.forEach((r) => {
        if (r.studentId) pendingRecord[r.studentId] = r;
      });
