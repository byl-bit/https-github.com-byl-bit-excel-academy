    ...
    <div className="space-y-8 animate-fade-in-up">
      ...
      <div className="space-y-4">
        <div className="flex items-center gap-3 px-1">
          ...
          <div>
            ...
            <p className="text-xs text-slate-500 font-medium">
              Direct applications from the 'Apply Now' form.
            </p>
          </div>
        </div>
        ...
        <div className="glass-panel rounded-2xl overflow-hidden shadow-sm">
          <div className="p-1">
            {admissionApplications.length === 0 ? (
              renderEmptyState("No new applications.")
            ) : (
              <div className="space-y-1">
                {admissionApplications
                  .slice(
                    (pageAdmissions - 1) * ITEMS_PER_PAGE,
                    pageAdmissions * ITEMS_PER_PAGE,
                  )
                  .map((app, idx) => (
                    <div
                      key={app.id}
                        ...
                        <div className="space-y-1">
                          ...
                          <div className="flex flex-wrap gap-x-4 gap-y-1 text-xs font-bold uppercase tracking-wider">
                            ...
                            <span className="text-slate-400 py-0.5 flex items-center gap-1">
                              <Users className="h-3 w-3" /> {app.familyFullName}
                            </span>
      ...
      <div className="space-y-4">
        <div className="flex flex-col md:flex-row justify-between items-center gap-4 px-1">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-slate-500 font-medium">
                Awaiting publication verification.
              </p>
            </div>
          </div>
          <Button
        ...
        <div className="glass-panel rounded-2xl overflow-hidden shadow-sm">
          <div className="p-1">
            {pendingResults.length === 0 ? (
              renderEmptyState("No pending results.")
            ) : (
              <div className="space-y-1">
                {sortedResults
                  .slice(
                    (pageResults - 1) * ITEMS_PER_PAGE,
                    pageResults * ITEMS_PER_PAGE,
                  )
                  .map((res, idx) => (
                    <div
                      key={res.key}
