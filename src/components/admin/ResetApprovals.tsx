    ...
    <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
      {requests.map((req) => {
        ...
        return (
          <Card
            key={req.id}
            className={`relative overflow-hidden border-none shadow-xl bg-white group hover:scale-[1.02] transition-all duration-300 ${isProcessing ? "opacity-70" : ""}`}
          >
              ...
              <div className="flex justify-between items-start mb-2">
                <div className="flex items-center gap-3">
                  ...
                  <div className="space-y-0.5">
                    <CardTitle className="text-lg font-black text-slate-800 tracking-tight leading-none">
                      {req.userName}
                    </CardTitle>
                ...
                {specificId && (
                  <div className="space-y-0.5">
                    <p className="text-[8px] font-black text-slate-400 uppercase tracking-widest flex items-center gap-1">
                      ...
                      {req.userRole === "student" ? "Student ID" : "Teacher ID"}
                    </p>
                    <p className="text-[11px] font-bold text-indigo-600 font-mono tracking-tighter">
                      {specificId}
                    </p>
                  </div>
                )}
