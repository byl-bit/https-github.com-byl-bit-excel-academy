...
export function ResultStats({ published, pending }: ResultStatsProps) {
  ...
  // Calculate pass rate
  const passed = published.filter(
    (r) => (r.promotedOrDetained || r.promoted_or_detained) === "PROMOTED",
  ).length;
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
      {stats.map((stat, i) => (
        <Card
          key={i}
          className="border-none shadow-sm glass-panel overflow-hidden"
        >
          <CardContent className="p-6">
            <div className="flex items-center justify-between">
              ...
              <div
                className={`h-12 w-12 rounded-2xl ${stat.bgColor} flex items-center justify-center shadow-inner`}
              >
