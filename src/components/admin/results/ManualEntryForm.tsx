...
export function ManualEntryForm({
  ...
  useEffect(() => {
    ...
    const existing = [...publishedResults, ...pendingResults].find(
      (r) => r.studentId === sid || r.student_id === sid,
    );
    ...
    if (existing) {
      ...
      setDecision(
        (existing.promotedOrDetained ||
          existing.promoted_or_detained ||
          "PROMOTED") as any,
      );
    } else {
      setMarks({});
  ...
  const grades = Array.from(
    new Set(students.map((s) => String(s.grade || ""))),
  ).sort();
  const sections = Array.from(
    new Set(students.map((s) => String(s.section || ""))),
  ).sort();
              ...
              {filteredStudents
                ...
                .map((s) => (
                  <option key={s.id} value={s.id}>
                    {s.name} - {s.studentId || (s as any).student_id || "No ID"}
                  </option>
                ))}
            </select>
          </div>
