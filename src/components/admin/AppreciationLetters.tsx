...
export function AppreciationLetters({
  students,
  results,
}: AppreciationLettersProps) {
  ...
  const [previewStudent, setPreviewStudent] = useState<PublishedResult | null>(
    null,
  );
  ...
  const eligibleResults = results.filter((r) => {
    ...
    const matchesGender =
      selectedGender === "all" ||
      normalizeGender(r.gender || (r as any).sex) === selectedGender;
  ...
  const generatePDF = async (
    ...
    doc.text("Certificate of Achievement", pageWidth / 2, 90, {
      align: "center",
    });
    ...
    doc.text("This certificate is proudly presented into", pageWidth / 2, 110, {
      align: "center",
    });
  ...
  const handleBulkDownload = async () => {
    ...
    try {
      ...
      for (const result of eligibleResults) {
        ...
        doc.setFontSize(14);
        doc.setTextColor(100, 100, 100);
        doc.text("Excellence in Education", pageWidth / 2, 60, {
          align: "center",
        });
        ...
        doc.setFont("helvetica", "normal");
        doc.text(
          "This certificate is proudly presented into",
          pageWidth / 2,
          110,
          { align: "center" },
        );
      ...
      doc.save(
        `Appreciation_Letters_Bulk_${new Date().toISOString().split("T")[0]}.pdf`,
      );
