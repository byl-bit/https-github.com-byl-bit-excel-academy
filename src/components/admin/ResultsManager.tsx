...
export function ResultsManager({
  ...
  const [view, setView] = useState<"directory" | "manual" | "subject">(
    "directory",
  );
  ...
  const sections = useMemo(
    () =>
      Array.from(new Set(students.map((s) => String(s.section || "")))).sort(),
    [students],
  );

  // Derived Data
  const filteredPublished = useMemo(() => {
    return (publishedResults as PublishedResult[])
      .filter((r) => {
        const matchesGrade = !filterGrade || String(r.grade) === filterGrade;
        const matchesSection =
          !filterSection || String(r.section) === filterSection;
        const matchesGender =
          !filterGender ||
          normalizeGender(r.gender || (r as any).sex) === filterGender;
        const matchesStatus =
          !filterStatus ||
          (r.promotedOrDetained || r.promoted_or_detained) === filterStatus;
        const matchesSearch =
          !search ||
        ...
        if (rows && rows.length > 1) {
          ...
          dataRows.forEach((row) => {
            ...
            headers.forEach((h, i) => {
              const val = row[i];
              if (
                h.toLowerCase() === "studentid" ||
                h.toLowerCase() === "student_id"
              ) {
                res.studentId = val;
              } else if (
                h.toLowerCase() === "name" ||
                h.toLowerCase() === "studentname"
              ) {
                res.studentName = val;
      ...
      <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-6">
        <div>
          ...
          <p className="text-slate-500 font-bold text-sm uppercase tracking-widest mt-2 ml-1">
            Centralized verified records & performance oversight
          </p>
        </div>
          ...
          {selectedSubject && filterGrade ? (
            <div className="bg-white rounded-3xl p-8 shadow-sm border border-slate-100 space-y-6">
              <div className="flex items-center gap-3">
                ...
                <h3 className="text-xl font-black text-slate-900">
                  {selectedSubject} Detailed Performance - Grade {filterGrade}{" "}
                  {filterSection && `(${filterSection})`}
                </h3>
              </div>
              <ResultTable
                ...
                students={students.filter((s) => {
                  ...
                  const matchesSection =
                    !filterSection || String(s.section) === filterSection;
                  const matchesGender =
                    !filterGender ||
                    normalizeGender(s.gender || (s as any).sex) ===
                      filterGender;
            ...
            <div className="py-20 text-center glass-panel rounded-3xl bg-slate-50/50 border-dashed border-2 border-slate-200">
              ...
              <p className="font-black text-slate-400 uppercase tracking-widest text-sm">
                Select both a Subject and a Grade to view detailed analytics
              </p>
            </div>
          )}
        </div>
