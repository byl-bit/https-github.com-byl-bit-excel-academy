...
export function PortalSidebarLayout({
        ...
        {role !== "admin" ? (
          ...
          >
            ...
            <Button
              ...
              className="shrink-0 text-slate-300 hover:text-red-500 hover:bg-red-50 rounded-xl h-8 w-8 transition-colors"
            >
  ...
  return (
    <div className="min-h-screen bg-slate-50/50 flex font-sans">
      ...
      {mobileOpen && (
        <div
          className="fixed inset-0 z-40 bg-slate-900/20 backdrop-blur-sm lg:hidden animate-in fade-in duration-200"
          onClick={() => setMobileOpen(false)}
        />
      )}
        ...
        <header className="h-16 bg-white/95 backdrop-blur-md border-b border-slate-200 sticky top-0 z-30 px-3 sm:px-8 flex items-center justify-between shadow-sm">
          <div className="flex items-center min-w-0 shrink-0">
            <Button
              variant="ghost"
              size="icon"
              className="lg:hidden text-slate-500 hover:bg-slate-50 mr-1 sm:mr-2 shrink-0"
            ...
            {role !== "admin" && (
              <div className="min-w-0 ml-1 sm:ml-2">
                <h2 className="text-sm sm:text-xl md:text-2xl font-black text-slate-800 tracking-tight truncate uppercase opacity-90">
                  {pageTitle}
                </h2>
              </div>
            )}
