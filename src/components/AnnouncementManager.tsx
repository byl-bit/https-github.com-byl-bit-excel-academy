      ...
      <CardContent>
        <div className="space-y-4">
          {announcements.map((item) => (
            <div
              key={item.id}
              className="flex items-start justify-between p-4 border rounded-lg hover:bg-muted/50 transition-colors text-left"
            >
                ...
                {item.media && item.media.length > 0 ? (
                  <div className="mt-3 grid grid-cols-2 sm:grid-cols-4 gap-3">
                    {item.media.map((m, idx) => (
                      <div
                        key={idx}
                        className="aspect-video relative rounded-lg overflow-hidden bg-slate-100 border border-slate-200"
                      >
                        {m.type === "image" ? (
                          // eslint-disable-next-line @next/next/no-img-element
                          <img
                            src={m.url}
                            alt={`${item.title} media ${idx}`}
